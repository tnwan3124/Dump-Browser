...
#GLOBAL CONSTANT
CC_PATH_LOCAL_STATE = os.path.normpath(r"%s\AppData\Local\CocCoc\Browser\User Data\Local State"%(os.environ['USERPROFILE']))
...
CC_PATH = os.path.normpath(r"%s\AppData\Local\CocCoc\Browser\User Data"%(os.environ['USERPROFILE']))

def get_secret_key():
    try:
        #(1) Get secretkey from Coccoc local state
        with open( CC_PATH_LOCAL_STATE, "r", encoding='utf-8') as f:
            local_state = f.read()
        ...
        print("%s"%str(e))
        print("[ERR] CC secretkey cannot be found")
        return None
...
def decrypt_password(ciphertext, secret_key):
    try:
        ...
        print("%s"%str(e))
        print("[ERR] Unable to decrypt, CC version <80 not supported. Please check.")
        return ""
    
def get_db_connection(CC_path_login_db):
    try:
        print(CC_path_login_db)
        shutil.copy2(CC_path_login_db, "Loginvault.db") 
        return sqlite3.connect("Loginvault.db")
    except Exception as e:
        print("%s"%str(e))
        print("[ERR] CC database cannot be found")
        return None
...
if __name__ == '__main__':
    try:
        ...
        with open('decrypted_password.csv', mode='w', newline='', encoding='utf-8') as decrypt_password_file:
            ...
            folders = [element for element in os.listdir(CC_PATH) if re.search("^Profile*|^Default$",element)!=None]
            for folder in folders:
            	#(2) Get ciphertext from sqlite database
                CC_path_login_db = os.path.normpath(r"%s\%s\Login Data"%(CC_PATH,folder))
                conn = get_db_connection(CC_path_login_db)
